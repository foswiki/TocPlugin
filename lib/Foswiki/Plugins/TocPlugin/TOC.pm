...
sub _processTag {
    ...
    return Foswiki::Plugins::TocPlugin::Section::_error("Bad tag $tag: " . join(",", @params));
}
...
sub _processTOCTags {
    ...
    $text =~ s/%(REF|REFTABLE){([^%]*)}%/&_processTag($toc, $wif, $toc->currentTopic, $1, Foswiki::Plugins::TocPlugin::Attrs->new($2))/geo;
...
sub _printWithTOCTags {
    ...
    # The order in which the other tags is done is irrelevant
    my $nullatt = Foswiki::Plugins::TocPlugin::Attrs->new("");
    $text =~ s/%(REF|REFTABLE){([^%]*)}%/&_processTag($toc, $wif, $ct, $1, Foswiki::Plugins::TocPlugin::Attrs->new($2))/geo;
    $text =~ s/%(CONTENTS)({[^%]*})?%/&_processTag($toc, $wif, $ct, $1, Foswiki::Plugins::TocPlugin::Attrs->new($2))/geo;
...
sub processTopic {
    ...
    if (!$toc || $web ne $toc->web() || $topic eq "WebOrder") {
        my $mess;
        ($toc, $mess) = Foswiki::Plugins::TocPlugin::TopLevelSection::createTOC($web, $wif);
        if ($toc && $topic eq "WebPrint") {
            return _webPrint($toc, $wif, $web);
        }

        return Foswiki::Plugins::TocPlugin::Section::replaceAllTags($text, $mess) unless $toc;
    }
    my $ct = $toc->currentTopic();
